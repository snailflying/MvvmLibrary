...
class IndicatorView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr),
     ...
     * indicator动画样式
     */
    private var indicatorAnimStyle = 0
...
//    @IntDef(IndicatorStyle.INDICATOR_CIRCLE, IndicatorStyle.INDICATOR_CIRCLE_RECT, IndicatorStyle.INDICATOR_BEZIER, IndicatorStyle.INDICATOR_DASH, IndicatorStyle.INDICATOR_BIG_CIRCLE)
    ...
    annotation class IndicatorAnimStyle {
        companion object {
    ...
    override fun getIndicatorParams(): FrameLayout.LayoutParams {
        if (mParams == null) {
            mParams = FrameLayout.LayoutParams(
                    ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT)
            mParams!!.gravity = Gravity.BOTTOM or Gravity.CENTER_HORIZONTAL
            mParams!!.bottomMargin = dip2px(8f)
        }
        return mParams!!
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (indicatorAnimStyle == IndicatorAnimStyle.INDICATOR_CIRCLE) {
            drawCircle(canvas, midY)
        } else if (indicatorAnimStyle == IndicatorAnimStyle.INDICATOR_CIRCLE_RECT) {
            drawCircleRect(canvas, midY)
        } else if (indicatorAnimStyle == IndicatorAnimStyle.INDICATOR_BEZIER) {
            drawBezier(canvas, midY)
        } else if (indicatorAnimStyle == IndicatorAnimStyle.INDICATOR_DASH) {
            drawDash(canvas, midY)
        } else if (indicatorAnimStyle == IndicatorAnimStyle.INDICATOR_BIG_CIRCLE) {
    ...
    private fun indicatorStartX(index: Int): Float {
         ...
         */return centerX + if (indicatorAnimStyle == IndicatorAnimStyle.INDICATOR_DASH) 0f else (ratioIndicatorRadius - ratioRadius) / 2
    }
    ...
    fun setIndicatorAnimStyle(@IndicatorAnimStyle indicatorAnimStyle: Int): IndicatorView {
        this.indicatorAnimStyle = indicatorAnimStyle
        return this
    }
    ...
    fun setIndicatorParams(params: FrameLayout.LayoutParams?): IndicatorView {
        this.mParams = params
        return this
    }
